...
export const schoolOpsHandlers = [
  ...
  http.get('/api/school-ops/teachers/:id', async ({ params }) => {
    await delay(200);
    const { id } = params;
    const teacher = db.teachers.find((t) => String(t.id) === id);
  ...
  http.post('/api/school-ops/teachers', async ({ request }) => {
    ...
    const newTeacher = {
      ...data,
      id: Date.now(), // Use numeric timestamp for id
      isActive: true,
  ...
  http.put('/api/school-ops/teachers/:id', async ({ request, params }) => {
    ...
    const { id } = params;
    const data = await request.json() as Partial<Teacher>;
    const index = db.teachers.findIndex((t) => String(t.id) === id);
  ...
  http.delete('/api/school-ops/teachers/:id', async ({ params }) => {
    ...
    const isAssigned = db.routines.some(r => String(r.teacherId) === id);
    if (isAssigned) {
  ...
  http.get('/api/school-ops/routines', async ({ request }) => {
    ...
    if (classId) result = result.filter(r => String(r.classId) === classId);
    if (teacherId) result = result.filter(r => String(r.teacherId) === teacherId);
  ...
  http.delete('/api/school-ops/routines/:id', async ({ params }) => {
    await delay(200);
    const { id } = params;
    const index = db.routines.findIndex(r => String(r.id) === id);
    if (index !== -1) {
