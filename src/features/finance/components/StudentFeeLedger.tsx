    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 transition-colors">
          <p className="text-gray-500 dark:text-gray-400 text-sm font-medium">Total Payable</p>
          <p className="text-2xl font-bold text-gray-900 dark:text-white mt-2">{formatCurrency(ledger.totalAmount)}</p>
          <p className="text-xs text-blue-600 dark:text-blue-400 mt-1">Academic Year {ledger.academicYear}</p>
        </div>

        <div className="bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 transition-colors">
          <p className="text-gray-500 dark:text-gray-400 text-sm font-medium">Paid So Far</p>
          <p className="text-2xl font-bold text-green-600 dark:text-green-400 mt-2">{formatCurrency(ledger.paidAmount)}</p>
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">{transactions.filter(t => t.status === 'Success').length} successful transactions</p>
        </div>

        <div className={cn(
          "p-4 sm:p-6 rounded-xl shadow-sm border transition-colors",
          isOverdue
          ...
          <div className="flex justify-between items-end">
            <div>
              <p className={cn(
                ...
                isOverdue ? 'text-red-700 dark:text-red-500' : 'text-gray-900 dark:text-white'
              )}>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden transition-colors">
        <div className="px-4 py-4 sm:px-6 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center bg-gray-50/50 dark:bg-gray-900/50">
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                {transactions.map((txn) => (
                  <tr key={txn.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm font-mono text-gray-500 dark:text-gray-500">
                      {txn.id}
                    </td>
                    ...
                    <td className="px-6 py-4 text-sm">
                      <span className={cn(
                        ...
                        txn.status === 'Success' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300' :
                          txn.status === 'Failed' ? 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300' :
